...
- `SECTIONS` — list of sections with one-line descriptions

---

## Planning Pass (do this before writing a single file)

Work through the scratchpad below mentally. The output of this pass informs the content you write into config.js and every section HTML file.

<scratchpad>
Think through:
1. What are the 3–5 core concepts that must land for TOPIC? Which sections carry them?
...
3. How should depth vary across sections? Which sections are foundation (read carefully), which are reference (skim and return)?
...
Apply these when writing section HTML files. They supplement — never replace — the principles in CLAUDE.md.
...
- **Why this matters** — opens the section, tied to GOAL (not "in this section we will…")
- **Core explanation** — theory with inline code examples
- **Hands-on exercise** — something the learner runs or builds themselves
- **Common mistake** — at least one `callout-warning` per section
- **Verify step or checklist** — how the learner confirms they got it

### The hands-on project
- Appears as milestones spread across sections, not a standalone section
- One real problem solved end-to-end
- Naturally exercises all major concepts — not a forced add-on
- Includes a troubleshooting sub-section for the top 2–3 failure modes
- Final section checklist = portfolio milestone items (3–5 items)
...
Use this to calibrate how much content to generate per section:

| Section role | Tag | Depth |
|---|---|---|
| Foundation / must-read | `Core concept`, `Read first` | 4–6 sub-sections, exercises, checklist |
| Hands-on setup | `Do first` | 3–4 sub-sections, verify step |
| Skill the learner uses repeatedly | `Core skill`, `Memorize this` | 3–5 sub-sections, worked example |
| Advanced, return later | `Month 2+`, `Week 3+` | 2–3 sub-sections, reference-style |
| For learners with prior knowledge | `Skip if confident` | 2–3 sub-sections, self-test |
...
1. `config.js` — sections list with tags, colors, stateKey, prerequisites, time/goal text
2. `index.html` — dashboard shell with tutorial overview woven into the subtitle/intro copy
3. `pages/section-N.html` for each section — full content per the structure above
4. `pages/settings.html` — copy from template
5. `states/manifest.json` — `{"states": []}`

The tutorial_overview and prerequisites from your planning pass belong in `index.html` intro copy and `config.js` prerequisites array, not in a separate document.
